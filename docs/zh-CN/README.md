...
NodePassDash 是一个现代化的 **NodePass** 管理面板，用于集中管理端点（Endpoints）、隧道（Tunnels）与服务（Services）。项目采用 **Go（Gin + GORM + SQLite/PostgreSQL）** 后端并内置 **React（Vite + TypeScript + HeroUI）** 前端，通过 **SSE / WebSocket** 提供实时监控与交互。
...
./nodepassdash --disable-sse-log         # 禁用 SSE 日志记录到文件
