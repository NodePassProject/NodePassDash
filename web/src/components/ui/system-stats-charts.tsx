...
const SingleChart = ({ title, value, color, chartData, icon, index }: any) => {
  ...
  // 调试信息
  React.useEffect(() => {
    console.log(`[SingleChart ${title}] 数据更新:`, { value, chartDataLength: chartData?.length, chartData: chartData?.slice(-3) });
        ...
        <div className="flex justify-between items-start px-4 pt-4">
          <div className="flex items-center gap-2">
            <div className={classes.iconWrapper()}>
              <Icon icon={icon} className="text-inherit" width={16} height={16} />
            </div>
...
// 双指标图表组件
const DualChart = ({ title, value1, value2, label1, label2, color, chartData, icon, index }: any) => {
  const classes = React.useMemo(() => chart({color}), [color]);
  
  // 调试信息
  React.useEffect(() => {
    console.log(`[DualChart ${title}] 数据更新:`, { value1, value2, chartDataLength: chartData?.length, chartData: chartData?.slice(-3) });
...
// 内存专用图表组件
const MemoryChart = ({ title, memUsed, memTotal, swapUsed, swapTotal, memPercent, swapPercent, color, chartData, icon, index }: any) => {
  const classes = React.useMemo(() => chart({color}), [color]);
  ...
  }, [title, memUsed, memTotal, swapUsed, swapTotal, memPercent, swapPercent, chartData]);
...
export default function SystemStatsCharts({ endpointId, isExperimentalMode = true, endpointOS, endpointVersion }: SystemStatsChartsProps) {
  ...
  const shouldConnect = isExperimentalMode && 
    endpointId && 
  ...
  const { latestData, isConnected } = useSystemMonitorWS(wsEndpointId, {
    ...
    onData: (data) => {
      ...
      setPreviousTimestamp(prevTime => {
        if (prevTime !== null && currentTimestamp > prevTime) {
          ...
          setPreviousNetRx(prevNetRx => {
            if (data.netrx !== undefined && prevNetRx !== null && timeDiff > 0) {
          ...
          setPreviousNetTx(prevNetTx => {
            if (data.nettx !== undefined && prevNetTx !== null && timeDiff > 0) {
          ...
          setPreviousDiskR(prevDiskR => {
            if (data.diskr !== undefined && prevDiskR !== null && timeDiff > 0) {
          ...
          setPreviousDiskW(prevDiskW => {
            if (data.diskw !== undefined && prevDiskW !== null && timeDiff > 0) {
      ...
      setDataHistory(prev => {
        ...
        console.log('[SystemStatsCharts] 历史数据更新，长度:', newHistory.length);
        return newHistory;
      });
    }
  });
  ...
  const currentCpuData = useMemo(() => {
    ...
    return {
      ...
      chartData: cpuChartData.length > 0 ? cpuChartData : [
        { time: "0", value: 0 },
        { time: "1", value: 0 }
      ],
      color: "primary",
  ...
  const currentRamData = useMemo(() => {
    const memUsed = latestData?.mem_total && latestData?.mem_free 
      ? latestData.mem_total - latestData.mem_free 
      : 0;
    const memTotal = latestData?.mem_total || 0;
    const swapUsed = latestData?.swap_total && latestData?.swap_free 
    ...
    return {
      ...
      chartData: ramChartData.length > 0 ? ramChartData : [
  ...
  }, [latestData?.ram, latestData?.swap, latestData?.mem_total, latestData?.mem_free, latestData?.swap_total, latestData?.swap_free, latestData?.timestamp, ramChartData]);
  ...
  const currentNetworkData = useMemo(() => ({
    ...
    label1: "上行",
    label2: "下行",
    chartData: networkChartData.length > 0 ? networkChartData : [
      { time: "0", value1: 0, value2: 0 },
      { time: "1", value1: 0, value2: 0 }
    ],
    color: "success",
  ...
  const currentDiskData = useMemo(() => ({
    ...
    label1: "读取",
    label2: "写入",
    chartData: diskChartData.length > 0 ? diskChartData : [
      { time: "0", value1: 0, value2: 0 },
      { time: "1", value1: 0, value2: 0 }
    ],
    color: "warning",
  ...
  }, [endpointId, isExperimentalMode, endpointOS, endpointVersion, shouldConnect, isConnected, dataHistory.length, latestData, netRxRate, netTxRate, diskRRate, diskWRate]);
  ...
  if (!shouldConnect) {
    ...
    if (isExperimentalMode && endpointId) {
      ...
      if (endpointOS?.toLowerCase() !== 'linux') {
        reasons.push(`操作系统不支持(当前: ${endpointOS || '未知'}, 需要: Linux)`);
      }
      if (!endpointVersion || !isVersionGreaterOrEqual(endpointVersion, '1.6.0')) {
        reasons.push(`版本不支持(当前: ${endpointVersion || '未知'}, 需要: >=1.6.0)`);
      }
