...
export default function TunnelDetailPage() {
  ...
  const getAddressOptions = React.useMemo(() => {
    ...
    const addresses = [];
    const mainAddr = `${tunnelInfo.targetAddress}:${tunnelInfo.targetPort}`;
    ...
    // 添加扩展目标地址
    if (tunnelInfo.extendTargetAddress && tunnelInfo.extendTargetAddress.length > 0) {
      tunnelInfo.extendTargetAddress.forEach((addr, index) => {
        addresses.push({
          value: addr,
  ...
  React.useEffect(() => {
    if (tcpingModalOpen && tunnelInfo && !tcpingLoading && !tcpingResult) {
      const mainAddress = `${tunnelInfo.targetAddress}:${tunnelInfo.targetPort}`;
      const hasExtendAddresses = tunnelInfo.extendTargetAddress && tunnelInfo.extendTargetAddress.length > 0;

      if (hasExtendAddresses) {
        // 有多个地址，默认选择第一个，等待用户确认
        if (!tcpingSelectedAddress) {
          setTcpingSelectedAddress(tunnelInfo.extendTargetAddress[0]);
        }
      } else {
        // 只有一个地址，直接开始测试
  ...
  }, [tcpingModalOpen, tunnelInfo, tcpingLoading, tcpingResult, performTcpingTest]);
