...
export function useSystemMonitorWS(
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const wsUrl = buildApiUrl(`/api/ws/system-monitor?endpointId=${endpointId}`).replace('http', 'ws');
      ...
      websocket.onmessage = (event) => {
        try {
          ...
          if (data.endpointId && data.info && data.info.os && data.info.arch) {
            ...
            console.log('[System Monitor WS] 检测到新的系统监控数据格式（info对象）');
            
            const info = data.info;
            
            // 计算RAM使用率（百分比）
            const ramUsedBytes = info.mem_total - info.mem_free;
            const ramUsagePercent = info.mem_total > 0 ? (ramUsedBytes / info.mem_total) * 100 : 0;
            
            // 计算Swap使用率（百分比）
            const swapUsedBytes = info.swap_total - info.swap_free;
            const swapUsagePercent = info.swap_total > 0 ? (swapUsedBytes / info.swap_total) * 100 : 0;
          ...
          } else if (data.os && data.arch && (data.cpu !== undefined || data.mem_total !== undefined)) {
            // 直接数据格式（兼容旧版本）
            console.log('[System Monitor WS] 检测到直接系统监控数据格式');
            
            // 计算RAM使用率（百分比）
            const ramUsedBytes = data.mem_total - data.mem_free;
            const ramUsagePercent = data.mem_total > 0 ? (ramUsedBytes / data.mem_total) * 100 : 0;
            
            // 计算Swap使用率（百分比）
            const swapUsedBytes = data.swap_total - data.swap_free;
            const swapUsagePercent = data.swap_total > 0 ? (swapUsedBytes / data.swap_total) * 100 : 0;
            ...
            console.log('[System Monitor WS] 系统监控数据(旧格式):', monitorData);
            setLatestData(monitorData);
          ...
          console.error('[System Monitor WS] 解析消息失败:', err, 'Raw data:', event.data);
        }
      };
      ...
      websocket.onclose = (event) => {
        console.log('[System Monitor WS] 连接已关闭:', event.code, event.reason);
        setIsConnected(false);
        setIsConnecting(false);
