...
import { faBolt, faEye, faEyeSlash, faChevronDown, faChevronUp } from "@fortawesome/free-solid-svg-icons";
 ...
 * 简单创建实例模态框（简易表单）
 */
export default function SimpleCreateTunnelModal({ isOpen, onOpenChange, onSaved, mode: modalMode = 'create', editData }: SimpleCreateTunnelModalProps) {
  ...
  const handleSubmit = async () => {
    const {
      apiEndpoint, type, tunnelName, tunnelAddress, tunnelPort,
      targetAddress, targetPort, tlsMode, logLevel, password, min, max,
      certPath, keyPath, mode, read, rate
    } = formData;
    ...
    if (type === 'client' && !['1', '2'].includes(mode)) {
      addToast({ title: "请选择有效的客户端模式", description: "客户端模式必须为模式1或模式2", color: "warning" });
      return;
    }
    ...
    // server + 2 校验证书路径
    if (type === 'server' && tlsMode === '2' && (!certPath.trim() || !keyPath.trim())) {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          certPath: type==='server' && tlsMode==='2' ? certPath.trim() : undefined,
          keyPath: type==='server' && tlsMode==='2' ? keyPath.trim() : undefined,
          logLevel: logLevel || undefined,
          password: password || undefined,
          min: type==='client' && min !== '' ? min : undefined,
          max: (type==='client' && max !== '') || (type==='server' && max !== '') ? max : undefined,
          // 新增字段
          mode: mode || undefined,
          read: read || undefined,
          rate: rate || undefined,
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <Select
                      ...
                      onSelectionChange={(keys)=> handleField('type', Array.from(keys)[0] as string)}
                      // isDisabled={modalMode==='edit'}
                    >
                  ...
                  <div className={`grid grid-cols-${formData.type === 'client' ? '1' : '2'} gap-2`}>
                  {/* 日志级别 */}
                    <Select
                      label="日志级别"
                    ...
                    >
                      <SelectItem key="inherit">
                        {(() => {
                          ...
                          return masterLog ? `继承 (${masterLog.toUpperCase()})` : "继承主控";
                        })()}
                      </SelectItem>
                  ...
                  {formData.type === 'server' && (
                    <Select
                      label="TLS 模式"
                      selectedKeys={formData.tlsMode ? [formData.tlsMode] : ['inherit']}
                      onSelectionChange={(keys)=> {
                        const selectedKey = Array.from(keys)[0] as string;
                        handleField('tlsMode', selectedKey === 'inherit' ? '' : selectedKey);
                      }}
                    >
                      <SelectItem key="inherit">
                        {(() => {
                          ...
                          return masterTls ? `继承 (${getTLSModeText(masterTls)})` : "继承主控";
                        })()}
                      </SelectItem>
                   ...
                   <div className="relative ">
                     ...
                     <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-background dark:bg-[#18181B] px-4">
                       ...
                       >
                         可选配置
                         <FontAwesomeIcon 
                           icon={isOptionalExpanded ? faChevronDown : faChevronUp} 
                           className="text-xs"
                         />
                       </button>
                     </div>
                      ...
                      {isOptionalExpanded && (
                        ...
                        >
                          <div className="space-y-4">
                            <div className={`grid grid-cols-${formData.type === 'client'&&formData.mode==='1' ? '1' : '2'} gap-2`}>
                              {/* 密码配置（可选） - 仅在选中主控版本不为空时显示 */}
                              {((formData.mode ==='2' && formData.type==='client' ) ||  formData.type==='server' ) && renderPasswordInput()}
